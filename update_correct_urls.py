# update_correct_urls.py
from app import app
from models import db, Word
import re


def update_correct_urls():
    with app.app_context():
        blob_urls = {}
        try:
            with open('blob_urls_correct.txt', 'r', encoding='utf-8') as f:
                for line in f:
                    if '=' in line:
                        key, value = line.strip().split('=', 1)
                        blob_urls[key.strip().lower()] = value.strip()
        except FileNotFoundError:
            print("❌ Файл blob_urls_correct.txt не найден!")
            return

        print(f"📸 Загружено {len(blob_urls)} ссылок")
